...
if(isset($_GET['cari'])){
		$cari = $_GET['cari'];
		$data = mysqli_query($koneksi,"select * from berita where judul like '%".$cari."%' ORDER BY id_berita DESC");
	if($_GET['module']==''){
	...
	while ($s=mysqli_fetch_array($sebelum)){
		...
		$data = mysqli_query($koneksi,"select * from berita order by id_berita");		
}
